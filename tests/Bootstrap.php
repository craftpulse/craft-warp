...
if ($craftBase === false || !file_exists($craftBase . '/craft')) {
    fwrite(STDERR, "Warp test bootstrap could not locate a Craft install at cwd={$craftBase}.\n");
    fwrite(STDERR, "Run tests from the playground Craft root via:\n");
    fwrite(STDERR, "  ddev craft pest -- --configuration=vendor/craftpulse/craft-warp/phpunit.xml.dist\n");
    exit(1);
}
