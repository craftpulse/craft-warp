...
deleting a user erases every Warp row about them in the same operation. That
covers an Article 17 erasure request with no extra step, and an Article 15
access request can be answered from the two tables filtered by the user's id.
...
- Sign-in history is kept for up to 90 days; the list of active devices is
  kept only while those sessions remain signed in.
